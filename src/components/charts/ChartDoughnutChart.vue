<!-- ChartDoughnutChart.vue - Doughnut Chart for KPIs and Portfolio Distribution -->
<template>
  <div class="w-full h-full">
    <Doughnut :data="chartData" :options="(chartOptions as any)" :plugins="plugins" />
  </div>
</template>
...
const plugins = computed<Plugin<'doughnut'>[]>(() => [
  {
  ...
  // Center text plugin
  {
    id: 'centerText',
    afterDraw: (chart) => {
      if (!props.centerText && !props.centerSubtext) return
      ...
      if (props.centerText) {
        ...
        ctx.fillText(props.centerText, centerX, props.centerSubtext ? centerY - 10 : centerY)
      }

      // Subtext
      if (props.centerSubtext) {
        ctx.fillStyle = 'rgb(255, 255, 255)' // white - for better readability
